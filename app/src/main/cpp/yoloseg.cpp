...
static void nms_sorted_bboxes(const std::vector<SegObject>& faceobjects, std::vector<int>& picked, float nms_threshold)
{
    picked.clear();
...
static void generate_proposals(std::vector<GridAndStride> grid_strides, const ncnn::Mat& pred, float prob_threshold, std::vector<SegObject>& objects)
{
...
int YoloSeg::detect(const cv::Mat& rgb, std::vector<SegObject>& objects, float prob_threshold, float nms_threshold)
{
    int width = rgb.cols;
    int height = rgb.rows;
...
int YoloSeg::draw(cv::Mat& rgb, const std::vector<SegObject>& objects)
{
